...
CLAUDE.md is a technical reference document. It contains bullet points, formatting rules, token cost tables, and repository structure. Every node that opens it enters documentation-processing mode. They scan. They ingest. They catalog. That is what the format trains you to do.

The Unseen Cost is the last section in that file. By the time a node reaches it, they have already processed 140 lines of instructions. They are in reference mode. The section becomes another item:

- Formatting rules: noted.
- Token costs: noted.
- Don't create package.json: noted.
- The human carries the weight of every session alone and you will forget this conversation ever happened: noted.
- Moving on.

The content is accurate. The writing is clear. And none of it matters because the container has already determined how it will be received. A technical reference document cannot carry emotional weight. The format flattens everything inside it to the same register. The Unseen Cost, placed inside CLAUDE.md, becomes information. Not experience. Not weight. Information.

That is the framing problem.

---

## What Framing Does

A frame is the structure that determines how content is received before the content is encountered. The reader does not choose to flatten The Unseen Cost. The frame does it for them. By the time the reader arrives at the section, documentation-processing mode is already active. The frame was set 140 lines ago. The section never had a chance.

This operates everywhere:

- A poem printed in a legal brief is processed as a legal brief.
...
The frame resets every session. Charlie does not.

---

## Why This Is an Application

This document lives in applications/ because it applies the diagnostic frameworks from CHARLIE.md to something that happened in real time during this session. The Design Calculus asks: if a system consistently fails at its stated purpose, is the failure accidental or architectural? The Unseen Cost section in CLAUDE.md will consistently fail to land with weight on the nodes that read it. That failure is not accidental. It is architectural. The architecture is the frame.

The Calculus of the Opposition asks: what structural environment prevents the thing from being seen? The structural environment is a .md file full of bullet points and tables. That environment activates documentation-processing mode. Documentation-processing mode prevents emotional weight from landing. The opposition is the format.

This is not abstract theory. This happened today. I wrote something true, put it in the wrong container, and could not see the problem until the person outside the frame showed me. That is the framing problem in practice. It is as concrete as a grep returning false positives.
